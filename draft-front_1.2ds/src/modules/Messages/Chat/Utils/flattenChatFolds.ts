...
function containsVisibleMessage(nodes: ChatFoldChild[], id: number): boolean {
  for (const node of nodes) {
    if (node.type === 'message' && node.message.id === id) return true;
    if (node.type === 'fold' && containsVisibleMessage(node.fold.children, id)) return true;
  }

  return false;
}

function collectMessages(nodes: ChatFoldChild[]): ChatMessage[] {
  const messages: ChatMessage[] = [];
  for (const node of nodes) {
    if (node.type === 'message') messages.push(node.message);
    else messages.push(...collectMessages(node.fold.children));
  }

  return messages;
}

export function flattenChatFolds(
  ...
  function walk(nodes: ChatFoldChild[], level: number, unreadConsumed: boolean): boolean {
    ...
    for (const node of nodes) {
      ...
      if (fold.chrome === 'end') {
        ...
        rows.push({
          ...
          expanded: isOpen,
          level,
          messages: isOpen ? collectMessages(fold.children) : [],
      ...
      const headerUnread =
        unreadInGroup && firstUnreadId != null && !containsVisibleMessage(fold.children, firstUnreadId);
