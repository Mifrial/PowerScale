      ...
      <div class="chat-main">
        <template v-if="store.activeChat">
          ...
          <div ref="messagesRef" class="chat-messages" @scroll="onMessagesScroll">
            ...
            <div
              v-for="msg in store.renderedMessages"
              :key="msg.id"
              class="chat-message"
              :class="{ own: msg.userId === auth.userId }"
            >
              ...
              <div v-if="msg.rolls.length" class="chat-rolls">
                <DiceRollResult v-for="(roll, ri) in msg.rolls" :key="ri" :roll="roll" :index="ri" />
              </div>
            </div>
          </div>
...
import { DateTime } from '@/modules/Core/Engine/Type/DateTime'
