...
export class ChatVisibilityService {
  isMessageVisible(message: ChatMessage, chat: Pick<Chat, 'members'>, viewerId: number, roles: ChatRole[]): boolean {
    ...
    const seesAll = roles.some(
      (role) => role.code === viewerRole && role.permissions.includes(CHAT_PERMISSION_SEE_ALL),
    );
