 ...
 */
export function canViewCharacter(user: User | null | undefined, character: Character): boolean {
  ...
  return canSeeSheet(user, character.visibility, ctx);
}

/** Право редактирования: только владелец (ТР §7 — редактирование без чужого подтверждения вне игры). */
export function canEditCharacter(user: User | null | undefined, character: Character): boolean {
  if (!user) return false;
