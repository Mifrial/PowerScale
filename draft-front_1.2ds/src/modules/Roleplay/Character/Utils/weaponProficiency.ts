 ...
 */
export function weaponFamilyLadder(rules: Rule[], domain: string | null | undefined): number[] | null {
  if (!domain) return null;
  const family = rules.find((rule) => rule.type === 'weapon_family' && (rule.code === domain || rule.name === domain));
 ...
 */
export function weaponProficiencyLevels(
 ...
 */
export function weaponMasteryEntries(
  ...
  for (const [family, level] of proficiencyLevels) {
    ...
    for (const item of rules) {
      ...
      result.push({ weaponName: item.name, value, valueLabel: new DimensionalNumber(value).toString(), bonus: level });
    }
  }
