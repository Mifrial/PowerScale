...
describe('weaponProfileViews (панель предмета редактора)', () => {
  it('пата: два профиля удара, значения урона/пробития от текущей Силы', () => {
    const views = itemWeaponProfilesService.weaponProfileViews(specOf('Пата'), context, resolveName);
  ...
  it('ручной арбалет: фикс. урон/пробитие (без формулы в скобках), дистанция от базы действия (3×10), дальнобойность', () => {
    const [shoot] = itemWeaponProfilesService.weaponProfileViews(specOf('Ручной арбалет'), context, resolveName);
  ...
  it('маленький щит: атакующий профиль из ShieldBlock.weapon_profiles', () => {
    const [strike] = itemWeaponProfilesService.weaponProfileViews(specOf('Маленький щит'), context, resolveName);
  ...
  it('не-оружие (доспех) — пусто', () => {
    expect(itemWeaponProfilesService.weaponProfileViews(specOf('Латный доспех'), context, resolveName)).toEqual([]);
    expect(itemWeaponProfilesService.weaponProfileViews(undefined, context, resolveName)).toEqual([]);
  });
});

describe('itemParamsView (параметры предмета)', () => {
  it('пата (оружие): вес, мин. сила, прочность, защита/эффективность блокирования', () => {
    const params = itemWeaponProfilesService.itemParamsView(specOf('Пата'), characteristicValues, ruleCatalog);
  ...
  it('ручной арбалет: параметры из WeaponBlock', () => {
    const params = itemWeaponProfilesService.itemParamsView(
      specOf('Ручной арбалет'),
      characteristicValues,
      ruleCatalog,
    );
  ...
  it('маленький щит: блок + макс. Ловкость/Реакция (формула [Сила])', () => {
    const params = itemWeaponProfilesService.itemParamsView(specOf('Маленький щит'), characteristicValues, ruleCatalog);
  ...
  it('латный доспех: вес, макс. ловкость, штраф к силе и слои защиты', () => {
    const params = itemWeaponProfilesService.itemParamsView(specOf('Латный доспех'), characteristicValues, ruleCatalog);
  ...
  it('не-снаряжение (зелье/undefined) — null', () => {
    expect(
      itemWeaponProfilesService.itemParamsView(specOf('Зелье концентрации'), characteristicValues, ruleCatalog),
    ).toBeNull();
    expect(itemWeaponProfilesService.itemParamsView(undefined, characteristicValues, ruleCatalog)).toBeNull();
  });
});
