...
const innateWeapon = (id: number | null, code: string, name: string, family: string): Rule =>
...
const fixtureRules: Rule[] = [
  innateWeapon(null, 'ruka', 'Рука', 'fam-hand'),
  innateWeapon(null, 'noga', 'Нога', 'fam-foot'),
  innateWeapon(null, 'claw', 'Коготь', 'fam-claw'),
  base(null, 'fam-hand', 'weapon_family', 'Рука', { costs: [1, 2, 4] }),
  base(null, 'fam-foot', 'weapon_family', 'Нога', { costs: [1, 2, 4] }),
  base(null, 'fam-claw', 'weapon_family', 'Коготь', { costs: [1, 2, 4] }),
  ...
  base(null, 'humanoid-body', 'ability', 'Гуманоидное тело', {
    type: 'trait',
  ...
  base(null, 'four-arms', 'ability', 'Четыре руки', {
    type: 'trait',
    zones: { os: { kind: 'array', levels_cost: [0] } },
    requirements: [],
  ...
  base(null, 'human', 'species', 'Человек', {
    parent_race_code: null,
    abilities: [{ ability_code: 'humanoid-body', automatic: true }],
  }),
  base(null, 'alierets', 'race', 'Алиерц', {
    parent_race_code: 'human',
    cost_os: 10,
    characteristics: [],
    abilities: [],
  }),
  base(null, 'fourhanded', 'race', 'Четырёхрукий', {
    parent_race_code: 'human',
    cost_os: 0,
    characteristics: [],
...
function sheet(
  ...
  return {
    raceRuleCode: 'alierets' as string | null,
...
describe('applyRacialInnateGear', () => {
  it('гуманоидная раса: рука×2, нога×2, экип, gifted владение ур.1', () => {
    ...
    const ruka = next.inventory.find((item) => item.ruleCode === 'ruka');
    const noga = next.inventory.find((item) => item.ruleCode === 'noga');

    expect(ruka).toMatchObject({ quantity: 2, equipped: true });
    expect(noga).toMatchObject({ quantity: 2, equipped: true });
    expect(next.abilities.filter((ability) => ability.ruleCode === 'vladenie-oruzhiem')).toEqual([
  ...
  it('вид без расы тоже даёт тело', () => {
    const next = racialInnateGearService.applyRacialInnateGear(sheet({ raceRuleCode: 'human' }), fixtureRules);

    expect(next.inventory.map((item) => item.ruleCode).sort()).toEqual(['noga', 'ruka']);
  });

  it('без расы — без естественного оружия', () => {
    const next = racialInnateGearService.applyRacialInnateGear(sheet({ raceRuleCode: null }), fixtureRules);
  ...
  it('раса перекрывает quantity того же item_code', () => {
    const next = racialInnateGearService.applyRacialInnateGear(sheet({ raceRuleCode: 'fourhanded' }), fixtureRules);
    const ruka = next.inventory.find((item) => item.ruleCode === 'ruka');

    expect(ruka?.quantity).toBe(4);
    expect(next.inventory.find((item) => item.ruleCode === 'noga')?.quantity).toBe(2);
  });

  it('мутационный innate не из расового гранта не снимается', () => {
    const next = racialInnateGearService.applyRacialInnateGear(
      sheet({
        inventory: [{ id: 9, ruleCode: 'claw', quantity: 1, equipped: true, modifierRuleCodes: [] }],
      }),
      fixtureRules,
    );

    expect(next.inventory.find((item) => item.ruleCode === 'claw')).toMatchObject({ id: 9, quantity: 1 });
  ...
  it('уже купленное владение семьи становится gifted (первый уровень бесплатный)', () => {
    const next = racialInnateGearService.applyRacialInnateGear(
      sheet({
        abilities: [{ ruleCode: 'vladenie-oruzhiem', level: 2, domain: 'Рука', domainCode: 'fam-hand', zone: 'or' }],
      }),
      fixtureRules,
    );
  ...
  it('смена расы снимает gifted ур.1 и оставляет купленное владение без gifted', () => {
    ...
    const afterDrop = racialInnateGearService.applyRacialInnateGear({ ...upgraded, raceRuleCode: null }, fixtureRules);
...
describe('естественное оружие в каталоге и обзоре', () => {
  ...
  function versionOf(overrides: Partial<CharacterVersion> = {}): CharacterVersion {
    return {
      ...
      spaceCode: 'razrabotka',
      rulesRevision: 5,
      raceRuleCode: ahtar?.code ?? null,
      characteristics: strength ? [{ ruleCode: strength.code, base: dim(5), modifiers: [] }] : [],
      resources: [],
      abilities: [],
  ...
  it('humanoid-body, рука и нога есть в ревизии; innate не в закупке', () => {
    ...
    expect(
      characterBuildService.buyItem(catalogBuild({ raceRuleCode: null }), ruka?.code ?? '', 1, ruleCatalog).inventory,
    ).toEqual([]);
  });
  ...
  it('fromVersion гуманоида материализует руки/ноги; gifted владение не списывает ОР', () => {
    ...
    expect(build.inventory.find((item) => item.ruleCode === ruka?.code)?.quantity).toBe(2);
    expect(build.inventory.find((item) => item.ruleCode === noga?.code)?.quantity).toBe(2);
