...
describe('canSeeSheet / visibleSheetSections', () => {
  ...
  it('владелец и super_admin видят всё', () => {
    ...
    expect(canSeeSheet(makeUser(1), [], owner)).toBe(true);
    expect(visibleSheetSections(makeUser(1), [], owner)).toEqual(SHEET_VISIBLE_SECTIONS);

    const admin = makeUser(9, { super_admin: true });
    expect(visibleSheetSections(admin, [], makeCtx(admin))).toEqual(SHEET_VISIBLE_SECTIONS);
  });

  it('без правил лист скрыт от постороннего', () => {
    const ctx = makeCtx(makeUser(5));
    expect(canSeeSheet(makeUser(5), [], ctx)).toBe(false);
    expect(visibleSheetSections(makeUser(5), [], ctx)).toEqual([]);
  });

  it('в игровом контексте аудитория all = участники (секции из правила)', () => {
    const ctx = makeCtx(makeUser(5));
    expect(visibleSheetSections(makeUser(5), brief, ctx)).toEqual(['shortDescription']);
  });
  ...
  it('правило для списка пользователей даёт секции только им', () => {
    ...
    expect(visibleSheetSections(makeUser(3), visibility, makeCtx(makeUser(3)))).toEqual(['inventory']);
    expect(canSeeSheet(makeUser(7), visibility, makeCtx(makeUser(7)))).toBe(false);
  });
  ...
  it('роль с fullAccess (ведущие) даёт полный доступ независимо от правил', () => {
    ...
    const ctx = makeCtx(makeUser(5));
    expect(visibleSheetSections(makeUser(5), [], ctx)).toEqual(SHEET_VISIBLE_SECTIONS);
    // другой игрок без правил — не видит
    expect(canSeeSheet(makeUser(7), [], makeCtx(makeUser(7)))).toBe(false);
  });
  ...
  it('зона аудитории gm требует роли', () => {
    ...
    expect(visibleSheetSections(makeUser(5), visibility, makeCtx(makeUser(5)))).toEqual(['fullDescription']);
    expect(visibleSheetSections(makeUser(7), visibility, makeCtx(makeUser(7)))).toEqual([]);
  });

  it('на standalone аудитория all требует character.view', () => {
    const ctx = makeCtx(makeUser(5), null);
    expect(visibleSheetSections(makeUser(5), brief, ctx)).toEqual([]);
    const withView = makeUser(5, { permissions: ['character.view'] });
    expect(visibleSheetSections(withView, brief, makeCtx(withView, null))).toEqual(['shortDescription']);
  });
});
