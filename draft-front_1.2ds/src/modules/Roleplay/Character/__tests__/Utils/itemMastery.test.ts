...
describe('itemMasteryView (блок владения в панели предмета)', () => {
  it('семья, лестница и правило владения из ревизии; уровень 0', () => {
    ...
    const view = itemMasteryView(knifeSpec, build().abilities, ruleCatalog);
  ...
  it('уровень берётся из экземпляра владения семьи', () => {
    ...
    const view = itemMasteryView(knifeSpec, build(abilities).abilities, ruleCatalog);
  ...
  it('прокачка через сервис: взять 1 → прокачать до 3 → снять', () => {
    let current = build();
    const mastery = itemMasteryView(knifeSpec, current.abilities, ruleCatalog);
    ...
    expect(weaponProficiencyLevels(current.abilities, ruleCatalog).get(mastery!.familyCode)).toBe(1);
    ...
    expect(weaponProficiencyLevels(current.abilities, ruleCatalog).get(mastery!.familyCode)).toBe(3);
    ...
    expect(weaponProficiencyLevels(current.abilities, ruleCatalog).get(mastery!.familyCode) ?? 0).toBe(0);
  });
  ...
  it('понижение до уровня 1 из существующего экземпляра (регрессия: addAbilityInstance no-op)', () => {
    ...
    expect(weaponProficiencyLevels(next.abilities, ruleCatalog).get(family!.code)).toBe(1);
  });

  it('предмет без семьи и без спекула → null', () => {
    const potion = ruleCatalog.find((r) => r.name === 'Зелье концентрации');
    expect(itemMasteryView(potion?.spec as ItemSpec, build().abilities, ruleCatalog)).toBeNull();
    expect(itemMasteryView(undefined, build().abilities, ruleCatalog)).toBeNull();
  });
});
