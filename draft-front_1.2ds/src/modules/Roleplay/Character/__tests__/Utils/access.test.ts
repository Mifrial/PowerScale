...
describe('canViewCharacter', () => {
  it('отказывает без авторизованного пользователя', () => {
    expect(canViewCharacter(null, makeCharacter(10))).toBe(false);
    expect(canViewCharacter(undefined, makeCharacter(10))).toBe(false);
  });

  it('разрешает владельцу без глобального права', () => {
    expect(canViewCharacter(makeUser({ id: 7 }), makeCharacter(7))).toBe(true);
  });

  it('разрешает чужому с правом character.view', () => {
    const user = makeUser({ permissions: ['character.view'] });

    expect(canViewCharacter(user, makeCharacter(7))).toBe(true);
  });

  it('отказывает чужому без права character.view', () => {
    expect(canViewCharacter(makeUser(), makeCharacter(7))).toBe(false);
  });

  it('super_admin обходит проверку для чужого персонажа', () => {
    expect(canViewCharacter(makeUser({ super_admin: true }), makeCharacter(7))).toBe(true);
  });
});

describe('canEditCharacter', () => {
  it('разрешает только владельцу', () => {
    expect(canEditCharacter(makeUser({ id: 7 }), makeCharacter(7))).toBe(true);
    expect(canEditCharacter(makeUser({ id: 8 }), makeCharacter(7))).toBe(false);
  });

  it('отказывает без авторизованного пользователя', () => {
    expect(canEditCharacter(null, makeCharacter(7))).toBe(false);
  });

  it('владелец редактирует даже без глобального права', () => {
    const user = makeUser({ id: 7, groups: [], permissions: [] });

    expect(canEditCharacter(user, makeCharacter(7))).toBe(true);
  });
});
