...
describe('checkAdvantageModifiersFromItems', () => {
  ...
  it('экипированный шлем: помеха на внимательность от предмета, не на восприятие и не на попадание', () => {
    ...
    expect(checkAdvantageModifiersFromItems(sheet, rules, { kind: 'characteristic', code: 'attention' })).toEqual([
      { source_code: plate.id, source_label: 'Латный доспех', delta: -1 },
    ]);
    expect(checkAdvantageModifiersFromItems(sheet, rules, { kind: 'characteristic', code: 'perception' })).toEqual([]);
    expect(checkAdvantageModifiersFromItems(sheet, rules, { kind: 'hit' })).toEqual([]);
  });

  it('не экипированный или без шлема — без помехи', () => {
    expect(
      checkAdvantageModifiersFromItems(version([{ ...helmed, equipped: false }]), rules, {
        kind: 'characteristic',
        code: 'attention',
      }),
    ).toEqual([]);
    expect(
      checkAdvantageModifiersFromItems(version([{ id: 1, ruleId: plate.id, quantity: 1, equipped: true }]), rules, {
        kind: 'characteristic',
        code: 'attention',
      }),
    ).toEqual([]);
