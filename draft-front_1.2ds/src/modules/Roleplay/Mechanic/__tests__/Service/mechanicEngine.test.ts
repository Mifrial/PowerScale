...
function binding(overrides: Partial<MechanicBinding>): MechanicBinding {
  return { ruleCode: 'code-x', mechanicId: null, mechanicPayload: null, ...overrides };
}
...
describe('MechanicEngine', () => {
  it('resolveActive резолвит механики через каталог (mechanicId → code@version)', () => {
    ...
    const bindings = [binding({ ruleCode: 'roll', mechanicId: 5 }), binding({ ruleCode: 'plain' })];
    const mechanics = [mechanic({ id: 5, code: 'm', version: '1.0.0' })];

    const resolved = engine.resolveActive(bindings, mechanics);
    expect(resolved).toHaveLength(1);
    expect(resolved[0].handler.code).toBe('m');
  });

  it('resolveActive фильтрует срез по includeCodes и добавляет extraRuleCodes вне фильтра', () => {
    const registry = new MechanicHandlerRegistry();
    ...
    const engine = new MechanicEngine(registry);
    const bindings = [binding({ ruleCode: 'one', mechanicId: 1 }), binding({ ruleCode: 'two', mechanicId: 2 })];
    const mechanics = [
      mechanic({ id: 1, code: 'a', version: '1.0.0' }),
      mechanic({ id: 2, code: 'b', version: '1.0.0' }),
    ];

    const resolved = engine.resolveActive(bindings, mechanics, { includeCodes: ['a'], extraRuleCodes: ['two'] });
  ...
  it('runEvent выполняет подписанных в порядке приоритета и мутирует контекст', () => {
    ...
    const bindings = [binding({ mechanicId: 1 }), binding({ mechanicId: 2 }), binding({ mechanicId: 3 })];
    const active = engine.resolveActive(bindings, mechanics);
    const context = { trace: [] };
...
describe('PurchaseSurchargeHandler (миграция на контекст)', () => {
  ...
  function run(abilities: AbilitySeed[], racialCodes: string[], payload: MechanicPayload | null) {
    const registry = new MechanicHandlerRegistry();
    registry.register(purchaseSurchargeHandler);
    const engine = new MechanicEngine(registry);
    const bindings = [binding({ mechanicId: 4, mechanicPayload: payload })];
    const mechanics = [mechanic({ id: 4, code: 'purchase_surcharge', version: '1.0.0' })];
    const context = contextOf(abilities, racialCodes);
    engine.runEvent(PURCHASE_SURCHARGE_EVENT, context, engine.resolveActive(bindings, mechanics));
