...
import { namedCheckSpec, rollJointCheck, rollNamedCheck, successesOf } from '@/modules/Roleplay/Game/Utils/checkRoll';
...
describe('checkRoll', () => {
  it('namedCheckSpec: пул = база, преимущества в spec', () => {
    const spec = namedCheckSpec('Сила', { base: 4, size: 1 }, 2, RULES);
  ...
  it('namedCheckSpec: actorKey уходит в spec', () => {
    const spec = namedCheckSpec('Сила', { base: 4, size: 0 }, 0, RULES, 'character:3');
    expect(spec.actorKey).toBe('character:3');
  });

  it('соло ask: слой проверки против заданной сложности', () => {
    const spec = namedCheckSpec('Сила', { base: 1, size: 0 }, 0, RULES);
    const result = rollNamedCheck(spec, 'check-strength', { base: 2, size: 0 }, rngOf(0), RULES, MECHANICS);
  ...
  it('соло from_state: сложность — значение состояния', () => {
    const spec = namedCheckSpec('Воля', { base: 1, size: 0 }, 0, RULES);
    const difficulty = { base: 3, size: 0 };
    const result = rollNamedCheck(spec, 'check-exhaustion', difficulty, rngOf(0), RULES, MECHANICS);
  ...
  it('pairwise: чужой размерный итог = сложность стороны', () => {
    const left = namedCheckSpec('А', { base: 1, size: 0 }, 0, RULES);
    const right = namedCheckSpec('Б', { base: 1, size: 1 }, 0, RULES);
    const rolled = rollJointCheck(left, right, CHECK_SIMPLE_CODE, rngOf(0, 0.5), RULES, MECHANICS);
    expect(rolled.left.check?.difficulty).toEqual(successesOf(rolled.right));
    expect(rolled.right.check?.difficulty).toEqual(successesOf(rolled.left));
