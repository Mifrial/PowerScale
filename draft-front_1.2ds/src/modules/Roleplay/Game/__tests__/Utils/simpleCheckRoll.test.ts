...
import { rollSimpleCheckZero, withSimpleCheckZero } from '@/modules/Roleplay/Game/Utils/simpleCheckRoll';
...
describe('simpleCheckRoll', () => {
  it('чат-бросок — простая проверка vs 0, с 6 и 1', () => {
    const result = rollSimpleCheckZero(
  ...
  it('withSimpleCheckZero не перезаписывает уже посчитанную проверку', () => {
    ...
    expect(withSimpleCheckZero(result).check?.check_code).toBe('check-strength');
  });
});
