...
describe('buildCombatChatFolds', () => {
  it('вкладывает атаку в ход в раунд; воля остаётся в атаке', () => {
    ...
    const forest = buildCombatChatFolds(messages);
  ...
  it('сообщения без thread остаются плоскими', () => {
    const forest = buildCombatChatFolds([msg(1, 'привет'), msg(2, 'пока')]);
  ...
  it('live раскрывает ход, завершённая атака свёрнута', () => {
    ...
    const rows = flattenChatFolds(buildCombatChatFolds(messages), (id) => live.has(id), null);
