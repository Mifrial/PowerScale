...
import { injuryDifficulty, rollInjury } from '@/modules/Roleplay/Game/Utils/injuryRoll';
...
describe('injuryInputFromAttack', () => {
  it('режущий не кладёт raw в leftover — только в рану', () => {
    const input = injuryInputFromAttack({
      hpDamage: 0,
      cuttingWound: 6,
      woundFromHit: null,
      overlayExhaustion: 1,
  ...
  it('истощение 7 с оверлея даёт сложность 1', () => {
    ...
    expect(injuryDifficulty(input, resolveInjuryProcedure([], []), 0)).toBe(1);
  });
  ...
  it('колющий РУ 4 добавляет 2 к сложности', async () => {
    ...
    const result = rollInjury(input, rngFromFaces([1, 1, 1, 1]), ruleCatalog, mechanics);
