...
describe('типы урона и увечье', () => {
  it('колющий даёт хук доп. сложности от РУ, рубящий — дельту эффективности (помеха)', async () => {
    ...
    const piercing = damageTypeHooksService.injuryHooksOf(
      damageTypeHooksService.resolveDamageTypeHooks('piercing', ruleCatalog, mechanics),
    );
    const slashing = damageTypeHooksService.injuryHooksOf(
      damageTypeHooksService.resolveDamageTypeHooks('slashing', ruleCatalog, mechanics),
    );
    ...
    expect(damageTypeHooksService.resolveDamageTypeHooks(null, ruleCatalog, mechanics)).toEqual([]);
  });

  it('сложность — max источников; истощение с 7, offset 6', () => {
    const procedure = resolveInjuryProcedure([], []);
    expect(
      injuryRollService.injuryDifficulty(
        { leftoverDamage: 9, woundStrength: 4, endurance: 3, exhaustion: 0, attackSr: 0 },
        procedure,
        0,
      ),
    ).toBe(3);
    expect(
      injuryRollService.injuryDifficulty(
        { leftoverDamage: 2, woundStrength: 0, endurance: 3, exhaustion: 6, attackSr: 0 },
        procedure,
        0,
      ),
    ).toBe(0);
    expect(
      injuryRollService.injuryDifficulty(
        { leftoverDamage: 2, woundStrength: 0, endurance: 3, exhaustion: 7, attackSr: 0 },
        procedure,
        0,
      ),
    ).toBe(1);
    expect(
      injuryRollService.injuryDifficulty(
        { leftoverDamage: 2, woundStrength: 0, endurance: 3, exhaustion: 8, attackSr: 0 },
        procedure,
        0,
      ),
    ).toBe(2);
  ...
  it('истощение 15 → 9; рана 20 → 10 — берём max, не сырое 15', () => {
    const procedure = resolveInjuryProcedure([], []);
    const breakdown = injuryRollService.injuryDifficultyBreakdown(
  ...
  it('сложность 0 — без броска, сила 0', async () => {
    const mechanics = await fetchMechanics();
    const result = injuryRollService.rollInjury(
  ...
  it('колющий добавляет floor(РУ/2) к сложности, не к пулу; рубящий — помеха', async () => {
    const mechanics = await fetchMechanics();
    const piercing = injuryRollService.rollInjury(
      {
        leftoverDamage: 0,
        woundStrength: 0,
        endurance: 1,
    ...
    const slashing = injuryRollService.rollInjury(
      {
        leftoverDamage: 0,
        woundStrength: 0,
        endurance: 1,
    ...
    const result = injuryRollService.rollInjury(
      {
        ...
        attackSr: 0,
        difficulty: 1,
        advantages: injuryRollService.manualInjuryAdvantages(1),
      },
