...
import { mockCreateMacro, mockDeleteMacro, mockGetMyMacros, mockUpdateMacro } from '@/modules/Roleplay/Game/Mock/mockMacros'
...
describe('mock macros', () => {
  it('возвращает сид текущего пользователя', async () => {
    ...
    expect(attack?.rolls[0]).toMatchObject({ rollFormula: '5d6', adv: 1, dieSize: 0, rollLabel: 'Удар 1', variableAdvantages: false })
  ...
  it('создаёт макрос с несколькими бросками', async () => {
    ...
    expect(macros.find(m => m.id === created.id)?.rolls[1]).toMatchObject({ rollFormula: '4d6', adv: -1, dieSize: 1, variableAdvantages: true })
  })
  ...
  it('обновляет броски макроса', async () => {
    const created = await mockCreateMacro({ name: 'Старое', textTemplate: 'Текст', rolls: [{ rollFormula: '4d6', efficiency: 3, adv: 0, dieSize: 0, variableAdvantages: false }] })
