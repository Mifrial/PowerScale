...
export class GameAccessService {
  private memberOf(user: User | null | undefined, members: GameMember[]): GameMember | null {
    if (!user) return null;
  ...
  canEditGame(user: User | null | undefined, detail: GameDetail): boolean {
    ...
    const member = this.memberOf(user, detail.members);
   ...
   */
  canModerateGame(user: User | null | undefined, detail: GameDetail): boolean {
    if (!user) return false;
    if (accessService.hasAnyPermission(user, ['game.edit_all'])) return true;
    const member = this.memberOf(user, detail.members);
    if (!member) return false;
   ...
   */
  canAddGameMember(user: User | null | undefined, detail: GameDetail): boolean {
    if (!user) return false;
    if (accessService.hasAnyPermission(user, ['game.edit_all'])) return true;
    const member = this.memberOf(user, detail.members);
    if (!member) return false;
