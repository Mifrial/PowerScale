...
export class RaceSpecService {
  ...
  patchCharacteristic(
    ...
    const list = spec.characteristics.map((c, i) => {
      if (i !== index) return c;
      const next: RaceCharacteristic =
        key === 'characteristic_code'
  ...
  patchCharacteristicBase(spec: RaceSpec, index: number, value: DimensionalNumberValue | null): RaceSpec {
    const list = spec.characteristics.map((c, i) => (i === index ? { ...c, base: value ?? { base: 3, size: 0 } } : c));
  ...
  removePurchaseLevel(spec: RaceSpec, index: number, levelIndex: number): RaceSpec {
    const list = spec.characteristics.map((c, i) =>
      i === index ? { ...c, purchase: (c.purchase ?? []).filter((_, j) => j !== levelIndex) } : c,
    );
  ...
  patchAbility(spec: RaceSpec, index: number, key: 'ability_code' | 'automatic', value: string | boolean): RaceSpec {
    const abilities = spec.abilities.map((a, i) => (i === index ? { ...a, [key]: value } : a));
