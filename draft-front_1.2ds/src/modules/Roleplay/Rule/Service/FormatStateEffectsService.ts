...
export class FormatStateEffectsService {
  periodicityLabel(value: StatePeriodicity | undefined): string {
    if (!value) return 'каждый ход';

    return formatPeriodicity(value.value, value.step);
  }

  decayLabel(value: StateDecay | undefined, nameByCode: (code: string) => string): string {
    if (!value) return '0';
    switch (value.kind) {
      case 'fixed':
  ...
  damageSourceLabel(damage: Extract<StateEffect, { type: 'damage_over_time' }>['damage']): string {
  ...
  stateEffectLabel(effect: StateEffect, nameByCode: (code: string) => string): string {
    ...
    return this.dotEffectLabel(effect, nameByCode);
  }
}
