...
function patchPurchaseLevel(
  index: number,
  levelIndex: number,
  key: 'cost' | 'value',
  value: number | DimensionalNumberValue | null,
) {
  inner.value = raceSpecService.patchPurchaseLevel(
    specWith(inner.value),
    index,
    levelIndex,
    key,
    value,
...
<template>
  <div>
    <div class="text-body-2 text-medium-emphasis mb-2">
      «Базовая» — фиксированное значение (дальше правит только дары черт). «Докупаемая» — минимум (за 0 ОС) + таблица
      закупки «за N ОС → значение».
    </div>
    <div v-for="(c, index) in inner" :key="index" class="pa-2 mb-2 rounded bg-accent">
      <div class="bg-surface rounded pa-2">
        <div class="d-flex ga-2 align-center mb-1">
          ...
          <v-btn icon size="small" color="error" variant="text" @click="removeCharacteristic(index)">
        ...
        <div v-else>
          ...
          <v-btn variant="text" color="primary" size="small" @click="addPurchaseLevel(index)">
    ...
    <v-btn variant="text" color="primary" size="small" @click="addCharacteristic">
      <v-icon start>mdi-plus</v-icon>
