    ...
    <div class="mt-4">
      ...
      <template v-if="inner.transition.mode === 'chain'">
        <div class="d-flex gap-2 mt-2">
          <ClampedNumberField
            :model-value="transitionMaxShift()"
          ...
          />
          <v-select
            :model-value="transitionDirection()"
      ...
      <template v-else>
        ...
        <div
          v-for="(edge, edgeIndex) in transitionEdges()"
          :key="`edge-${edgeIndex}`"
          class="d-flex gap-2 mb-1"
        >
          <v-autocomplete
...
function patchStepCost(stepIndex: number, costIndex: number, key: string, value: unknown) {
...
function transitionMaxShift(): number {
  return inner.value.transition.mode === 'chain' ? inner.value.transition.max_shift : 1
}

function transitionDirection(): 'forward' | 'both' {
  return inner.value.transition.mode === 'chain' ? (inner.value.transition.direction ?? 'both') : 'both'
}

function transitionEdges(): { from: string; to: string }[] {
  return inner.value.transition.mode === 'custom' ? inner.value.transition.edges ?? [] : []
}
