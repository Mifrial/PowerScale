...
watch(subtypes, (newSubtypes) => {
  if (newSubtypes.includes('weapon')) {
    itemSpecService.ensureWeapon(draft.value);
  }
  if (newSubtypes.includes('armor')) {
    itemSpecService.ensureArmor(draft.value);
  }
  if (newSubtypes.includes('shield')) {
    itemSpecService.ensureShield(draft.value);
  }
});

const specToEmit = computed<ItemSpec>(() => itemSpecService.prune(draft.value, subtypes.value));
...
function updateWeapon(value: WeaponBlock) {
  draft.value.weapon = value;
}

function updateArmor(value: ArmorBlock) {
  draft.value.armor = value;
}
  ...
  <div>
    <v-expansion-panels v-model="expandedPanels" multiple>
      ...
      <v-expansion-panel value="item">
        ...
        <v-expansion-panel-text>
          <v-switch v-model="draft.innate" label="Естественный предмет" color="primary" hide-details />
          ...
          <div class="d-flex gap-2 mb-2">
            <ClampedNumberField
              ...
              style="flex: 1 1 auto"
            />
            <DimensionalNumberInput v-model="draft.weight" label="Вес" />
          </div>

          <v-autocomplete
            v-model="draft.special_rule_codes"
            :items="simpleRules"
            item-title="name"
            item-value="code"
            label="Спецправила"
