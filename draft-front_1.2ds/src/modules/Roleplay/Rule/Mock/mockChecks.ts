...
export const mockChecks: Rule[] = [
  ...
  checkRule(
    ...
    CHECK_INSIGHT_CODE,
    'Проницательность',
    'Проверка на общение: распознать ложь, намерения, настроение.',
    {
      type: 'check',
      parent_check_code: CHECK_COMMUNICATION_CODE,
      ...askBoth,
  ...
  checkRule(9021, CHECK_DEXTERITY_CODE, 'Проверка на Ловкость', 'Проверка на Ловкость.', {
    type: 'check',
    parent_check_code: CHECK_SIMPLE_CODE,
    characteristic_code: 'dexterity',
    ...askBoth,
  ...
  checkRule(
    9035,
    CHECK_FINE_MOTOR_CODE,
    'Проверка на мелкую моторику',
    'Наследник проверки на ловкость: медицина, воровство, фокусы и подобное. Характеристику на запуске можно подменить (например, Магией).',
    {
      type: 'check',
      parent_check_code: CHECK_DEXTERITY_CODE,
      characteristic_code: 'dexterity',
      allow_characteristic_override: true,
      ...askBoth,
    },
  ...
  checkRule(
    ...
    CHECK_KNOWLEDGE_CODE,
    'Проверка знания',
    'Память против сложности факта. Полоса и слоты ситуации задаются на запуске; характеристику подменять нельзя.',
    {
      type: 'check',
      parent_check_code: CHECK_SIMPLE_CODE,
