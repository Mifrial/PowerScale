...
import type { AbilitySpecDraft, Grant, Requirement, Formula, AbilityType } from '@/modules/Roleplay/Rule/Interface/abilityTypes'
import {
  ABILITY_TYPE_LABELS,
...
const ruleTagCodes = computed(() => {
  const ids = props.rule.tagIds ?? []
  return props.tags.filter(t => ids.includes(t.id)).map(t => t.code)
})

const type = computed<AbilityType | null>(() => {
  const s = spec.value
  if (s?.type) return s.type
  return resolveAbilityTypeFromTags(ruleTagCodes.value)
})
...
function reqText(req: Requirement): string {
  switch (req.type) {
    ...
    case 'has_ability_tag':
      return `${req.min_count} способность(ей) с признаком «${req.tag_code}»`
    case 'has_tag':
      return `признак «${req.tag_code}»`
...
function grantLabel(grant: Grant): string {
  switch (grant.type) {
    ...
    case 'tag':
      return grant.remove ? `Убирает признак «${grant.tag_code}»` : `Добавляет признак «${grant.tag_code}»`
    case 'item':
