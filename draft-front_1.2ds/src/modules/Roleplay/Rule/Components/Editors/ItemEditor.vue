...
import type {
  ...
  ResistanceSlot,
  CharacteristicLimit,
} from '@/modules/Roleplay/Rule/Interface/itemTypes'
import { ITEM_SUBTYPES, pruneItemSpecBySubtypes } from '@/modules/Roleplay/Rule/Interface/itemTypes'
...
const sources = computed(() => {
  return sourceStore.sources as { id: number; name: string }[]
})
...
const specToEmit = computed<ItemSpec>(() => pruneItemSpecBySubtypes(spec.value, subtypes.value))
...
onMounted(async () => {
  if (props.spec) {
    spec.value = JSON.parse(JSON.stringify(props.spec))

    if (props.spec.weapon) {
      subtypes.value.push('weapon')
    }
    if (props.spec.armor) {
      subtypes.value.push('armor')
    }
    if (props.spec.shield) {
