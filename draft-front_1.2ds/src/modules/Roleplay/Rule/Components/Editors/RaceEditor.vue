...
import type {
  ...
  RaceCharacteristic,
  RacePurchaseLevel,
  RaceCharacteristicMode,
} from '@/modules/Roleplay/Rule/Interface/raceTypes'
import {
  createEmptyRaceSpec,
  collectInheritedAbilities,
} from '@/modules/Roleplay/Rule/Interface/raceTypes'
import type { InheritedAbilityRef } from '@/modules/Roleplay/Rule/Interface/raceTypes'
...
const innerSpec = ref<RaceSpec>(createEmptyRaceSpec())
...
const characteristicOptions = computed(() =>
  spaceRules.value
    .filter((r: Rule) => r.type === 'characteristic' && r.spaceId === props.spaceId && !r.spec?.formula)
    .map(r => ({ code: r.code, name: r.name }))
)
...
const inheritedAbilities = computed<InheritedAbilityRef[]>(() => {
  ...
  return collectInheritedAbilities(innerSpec.value.parent_race_code, byCode)
})
...
const specToEmit = computed<RaceSpec>(() => JSON.parse(JSON.stringify(innerSpec.value)))
