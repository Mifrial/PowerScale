...
import {
  checkAncestorCodes,
  checkMatchesGrant,
  resolveCheckCharacteristicCode,
...
describe('checkResolution', () => {
  it('предки обмана: обман → общение → простая', () => {
    expect(checkAncestorCodes('deception', catalog)).toEqual([
      'deception',
      CHECK_COMMUNICATION_CODE,
      CHECK_SIMPLE_CODE,
    ]);
  ...
  it('грант на общение матчит обман; грант на простую — не увечье', () => {
    expect(checkMatchesGrant('deception', CHECK_COMMUNICATION_CODE, catalog)).toBe(true);
    expect(checkMatchesGrant('deception', CHECK_SIMPLE_CODE, catalog)).toBe(true);
    expect(checkMatchesGrant(CHECK_INJURY_CODE, CHECK_SIMPLE_CODE, catalog)).toBe(false);
  });

  it('пул обмана — Красноречие; подмена на запуске побеждает', () => {
    expect(resolveCheckCharacteristicCode('deception', catalog)).toBe('communication');
  ...
  it('код проверки характеристики: check-strength, иначе простая', () => {
    ...
    expect(resolveCheckCodeForCharacteristic('strength', withStrength)).toBe('check-strength');
    expect(resolveCheckCodeForCharacteristic('unknown', withStrength)).toBe(CHECK_SIMPLE_CODE);
  });
});
