 ...
 */
export function aggregateSourceDeltas<T extends { source_code: string | null; delta: number }>(
  entries: readonly T[],
): T[] {
...
export function netSourceDelta(entries: readonly { source_code: string | null; delta: number }[]): number {
  return aggregateSourceDeltas(entries).reduce((sum, entry) => sum + entry.delta, 0);
}

export function advantageEntries(
  delta: number,
