...
// Полные данные пользователей хранятся в Core/User/Service/mockUsers.ts.
// Пароли не должны храниться на клиенте — это только mock для тестирования.
const mockUsers: MockUser[] = [
  { id: 1, name: 'Иван', surname: 'Петров', nickname: 'IvanTheGreat', login: 'ivan_p', email: 'player@test.com', password: 'test', groups: ['Игрок'], registered: '01.01.2026', active: true, lastLogin: '27.07.2026 18:30' },
  { id: 2, name: 'Администратор', login: 'admin', email: 'admin@test.com', password: 'test', groups: ['Администратор', 'Игрок'], registered: '01.01.2025', active: true, lastLogin: '28.07.2026 09:00' },
  { id: 3, name: 'Анна', surname: 'Смирнова', nickname: 'Annet', login: 'anna_s', email: 'gm@test.com', password: 'test', groups: ['Ведущий', 'Игрок'], registered: '15.02.2026', active: true, lastLogin: '27.07.2026 22:15' },
]
...
export async function mockLogin(email: string, password: string): Promise<Omit<MockUser, 'password'>> {
  await delay()
...
export async function mockRegister(login: string, email: string, password: string): Promise<Omit<MockUser, 'password'>> {
  await delay()
...
export async function mockFindUser(loginOrEmail: string): Promise<Omit<MockUser, 'password'> | null> {
  await delay()
...
export async function mockGetCurrentUser(): Promise<Omit<MockUser, 'password'> | null> {
  await delay()
