      ...
      <v-row align="center" justify="center">
        <v-col cols="12" sm="8" md="4" lg="3">
          <v-card class="pa-6 card-translucent">
            <v-card-title class="text-center text-h4 font-weight-bold pa-0 mb-1 card-header"> Вход </v-card-title>
            <v-card-subtitle class="text-center text-body-2 mb-8 card-header"> Войдите в аккаунт </v-card-subtitle>
            ...
            <v-card-text class="pa-0">
              <v-form ref="formRef" @submit.prevent="handleLogin">
                ...
                <PasswordField v-model="password" label="Пароль" autocomplete="current-password" />

                <v-switch v-model="remember" label="Запомнить меня" color="primary" hide-details class="mb-3" />

                <Transition name="btn-swap" mode="out-in">
                  <div v-if="canLogin" key="login-layout" class="d-flex align-center ga-3 mb-3">
                    <v-btn type="submit" color="primary" size="large" class="flex-grow-1" :loading="auth.loading">
                      Войти
                    </v-btn>
                ...
                <div class="text-center text-body-2">
                  ...
                  <router-link to="/register" class="text-primary text-decoration-none"> Регистрация </router-link>
                </div>
              </v-form>
            </v-card-text>
          </v-card>
