...
export const users: User[] = [
  ...
  { id: 2, name: 'Администратор', login: 'admin', email: 'admin@test.com', groups: ['Администратор', 'Игрок'], registered: '01.01.2025', active: true, lastLogin: '28.07.2026 09:00' },
  ...
  { id: 7, name: 'Ольга', surname: 'Новикова', nickname: 'OlyaN', login: 'olga_n', email: 'olga@test.com', groups: ['Администратор', 'Ведущий', 'Игрок'], registered: '01.03.2025', active: true, lastLogin: '28.07.2026 08:30' },
...
export async function mockDeactivateUser(id: number, _reason?: string, _deactivatedUntil?: string, signal?: AbortSignal): Promise<void> {
  await delay(150, signal)
  const u = users.find(x => x.id === id)
  if (!u) throw new Error('User not found')
  u.active = false
