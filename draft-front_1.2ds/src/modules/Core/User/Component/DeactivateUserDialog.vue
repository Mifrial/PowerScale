...
<template>
  <v-dialog v-model="open" max-width="460">
    <v-card>
      ...
      <v-card-subtitle class="text-body-2"> Пользователь «{{ props.user.name }}» будет отключён </v-card-subtitle>
      <v-card-text>
        <v-textarea
          v-model="reason"
          label="Причина"
          ...
          hide-details
        />
        <v-text-field v-model="until" label="Отключён до (опционально)" type="date" variant="outlined" hide-details />
      </v-card-text>
      <v-card-actions>
        <v-spacer />
