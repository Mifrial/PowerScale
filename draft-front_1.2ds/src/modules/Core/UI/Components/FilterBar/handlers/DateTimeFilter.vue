...
import DateTimeInput from '@/modules/Core/UI/Components/Input/DateTimeInput.vue'
...
function parse(v: any) {
  ...
  mode.value = v.mode || 'equals'
  if (v.mode === 'equals') {
    singleValue.value = v.value
  } else if (v.mode === 'from') {
    fromValue.value = v.from
  } else if (v.mode === 'to') {
    toValue.value = v.to
...
function emitValue() {
  if (mode.value === 'equals') {
    ...
    const result: any = { mode: 'interval' }
...
function onFromChange(v: any) {
  fromValue.value = v
  emitValue()
}

function onToChange(v: any) {
  toValue.value = v
  emitValue()
}
