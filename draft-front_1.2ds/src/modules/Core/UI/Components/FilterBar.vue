...
const editBuffer = reactive<Record<string, any>>({})
...
function chipLabel(field: FilterField): string {
  ...
  if (typeof v === 'object' && v !== null) {
    ...
    if (v.mode === 'equals' || v.mode === undefined) return `${field.label}: ${dt ? formatDatetime(v.value ?? '') : v.value ?? ''}`
    if (v.mode === 'from') return `${field.label}: с ${dt ? formatDatetime(v.from ?? '') : v.from ?? '...'}`
    if (v.mode === 'to') return `${field.label}: до ${dt ? formatDatetime(v.to ?? '') : v.to ?? '...'}`
    if (v.mode === 'interval') {
      const parts: string[] = []
      if (v.from !== undefined && v.from !== null) parts.push(`с ${dt ? formatDatetime(v.from) : v.from}`)
      if (v.to !== undefined && v.to !== null) parts.push(`до ${dt ? formatDatetime(v.to) : v.to}`)
...
function onValueUpdate(key: string, value: any) {
