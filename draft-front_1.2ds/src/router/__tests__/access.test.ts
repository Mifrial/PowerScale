...
beforeEach(() => {
  ...
  registerAdminSection({ id: 'groups', title: 'Группы', to: '/admin/groups', icon: 'mdi-account-group', permission: 'user_group.view' })
  registerAdminSection({ id: 'keywords', title: 'Признаки', to: '/admin/keywords', icon: 'mdi-tag-multiple', permission: 'keyword.view' })
})

function makeTo(partial: Partial<{ meta: Record<string, unknown>; name: unknown; params: Record<string, string> }>): RouteLocationNormalized {
  return {
    meta: partial.meta ?? {},
...
const user = (id: number, permissions: string[], superAdmin = false): User => ({
  id, name: `U${id}`, login: `u${id}`, email: `u${id}@t`, groups: [], registered: '', active: true,
