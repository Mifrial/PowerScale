...
interface ProcessSpec {
  ...
  exit_step_codes?: string[]      // шаги, с которых доступен бесплатный выход; undefined = любой шаг
  transition: ProcessTransition
  failure?: 'restart_from_first' | 'end_action' | null  // поведение при провале шага (карточка; логика — позже)
  completion_effects?: ActionEffect[] // эффекты, добавляемые только после завершения процесса
}
```
...
- Процесс хранит отдельное сессионное состояние участника: правило процесса, текущий шаг и статус
  `active`. Пока сессия активна, обычные действия участника недоступны; следующий шаг выбирается
  отдельным действием игрока.
- Завершение процесса не тратит ресурс по умолчанию. `exit_step_codes` ограничивает шаги, с которых
  доступен ручной выход; отсутствие поля означает выход с любого шага. `completion_effects` применяются
  только при ручном завершении или завершении по провалу.
...
### 4.8 Дерево секций каталога развития

Секции являются навигационной таксономией, а не типом правила. Список секций хранится в
`SpaceRevision` и содержит `code`, `name`, `parentCode` и `sortOrder`. Способность ссылается
на секцию через `Rule.catalogSection` и задаёт `Rule.catalogSortOrder` для порядка внутри неё. Выбор
родительской секции в фильтре включает все дочерние секции; дерево улучшений по `parentCode`
остаётся отдельной структурой.

### 4.9 AttackAction

Атакующая способность и атакующий шаг процесса перед запуском преобразуются в единый
`AttackAction`. Его `source` хранит обычное правило или контекст сессии процесса,
`totalOdCost` — итоговую стоимость после временных модификаторов, а `strikes` — список
произвольной длины. По умолчанию форма создаёт один слот, дополнительные слоты добавляются
явно; искусственного ограничения на количество ударов нет.

Все удары первой версии имеют режим `simultaneous`: атакующий бросает попадание отдельно
для каждого профиля, защитник выполняет один общий бросок реакции против группы. Стоимость
реакции не может превышать итоговую стоимость атаки. Выбранный процессный шаг не заменяется
«Простым ударом»: он передаётся в `AttackAction` вместе с `ProcessActionContext`, а после
результата вызывается переход сессии процесса.

### 4.10 Заклинание (SpellSpec)
