...
### 4.1 Цена живёт в зонах (валюта = зона)

Этапы закупки = зоны видимости. Валюта определяется зоной, отдельно валюту не указываем:
- `creation` → ОС (Очки Создания) — расы и врождённые черты
- `personality` → ОЛ (Очки Личности) — особенности личности
- `live` → ОР (Очки Развития) — навыки и черты
...
- `zones: Partial<Record<ZoneId, AbilityCost>>` — каждая зона со своей ценой в своей валюте.
