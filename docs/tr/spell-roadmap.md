...
M9 закрыт: провал check → 2d6, состояние на ядре, аркан при дубле, `stability` Генератора.

Дальше по магии не открывать кристалл/ману и disruption «про запас». Сначала соседняя линия — редакционный проход уже выгруженного каталога ([`rule-content-roadmap.md`](rule-content-roadmap.md)): карточке можно верить, очередь runtime отдельно. Game дорабатываем **по очереди**, не по слепому моку. Backend магии — **M10** после заморозки контракта, которым Game уже пользуется, до большой новой выгрузки.
