...
30. ✅ Теги как способ определения подтипов способностей
    ...
    30.48. ✅ Игровой процесс — отдельная сессионная запись участника (`processRuleId`, `currentStepCode`, `active`), выбор следующего шага отдельным действием, бесплатный выход по умолчанию, блокировка обычных действий на время процесса; завершение может добавить `completion_effects`
    30.49. ✅ Атака — единый `AttackAction` для обычной способности и шага процесса: произвольное число одновременных ударов с профилем и целью каждого, одна общая реакция защиты, итоговая стоимость учитывает временные модификаторы, после броска сохраняется переход процесса
    30.50. ✅ Навигация каталога — дерево секций (`AbilitySection`) хранится в снимке ревизии, правила сортируются внутри секций через `Rule.catalogSortOrder`, а фильтр «Раздел» поддерживает выбор родительской секции вместе с потомками
    30.51. ⏸️ Отложено: исследовать и устранить подлагивания при раскрытии дерева секций в редакторах; проверить жизненный цикл `v-treeview`, объём перерендера и синхронизацию `opened`/`selected`
...
20.1. **Ожидание и прерывание процессов** — действие с кодом `wait` тратит выбранное количество доступных ОД; «Передать ход» при ненулевом остатке требует подтверждения, прерывает активный процесс, выполняет «Ожидание» через тот же исполнитель и только затем передаёт ход. Каждый шаг процесса обязан объявлять `interruption.mode`: `normal` или `emergency`; экстренная ветка хранит специальный эффект и пока не исполняется.
...
31. **Временные эффекты действий (2026-08-27):** `AbilitySpec.action_effects` — типизированные эффекты текущего/следующего действия и длительные эффекты до траты ресурса; области атаки поддерживают первые N или все элементы. Активные эффекты хранятся отдельным агрегатом Game API, не смешиваются с уже примененным `GameCombatOverlay`; проверка лимита ОД выполняется по итоговой стоимости после временных модификаторов.
