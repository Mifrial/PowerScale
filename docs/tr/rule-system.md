...
`keyword` — канонический термин для признака правила. Справочник — модуль `Roleplay/Keyword` ([`keyword-plan-01.md`](keyword-plan-01.md), HTTP — [`keyword-plan-02.md`](keyword-plan-02.md)), отделён от type-specific ссылок и участвует в фильтрации каталога и редакторе. В UI пользователю показывается «признак»/«Признаки», не «тег» (`DEC-062`). URL `/admin/keywords` и ключи `keyword.*` в этом контуре не переименовываются; оболочка админки User — эскиз, не канон «признак = админка».

Механика — модуль `Roleplay/Mechanic` ([`mechanic-plan-01.md`](mechanic-plan-01.md)): строка справочника хендлера; правило ссылается `mechanic_id`. Хендлеры Engine — тот же модуль, когда закроют OPEN.
...
Персистентность часов (identity, экземпляры, space, revision, состав) — универсальный модуль `Versioning/Space` ([`versioning-roadmap.md`](versioning-roadmap.md), `DEC-080`). Продуктовый оператор — `Roleplay/RuleSpace`. Секции каталога и `code` пространства — не поля универсального Space. Rule не импортирует RuleSpace/Versioning.
...
`SpaceRevision` — опубликованный набор правил. `revision` — числовой номер, уникальный внутри пространства. Frontend передаёт `(spaceId, revision)`. Backend разрешает пару; `publishedAt` иммутабелен. Срез на сервере — состав ревизии (указатели на экземпляры), не сканирование `created_at <= publishedAt` по всей ленте.

`RuleVersion` immutable. Коммит ревизии атомарен. Timestamp коммита не должен включать версии будущей ревизии.

### Draft и контекст

Draft — редакторское состояние до публикации. Контекст пространства и ревизии передаётся через URL/route или публичный context API; Rule не импортирует RuleSpace и не читает его store напрямую.
...
Общий редакционный `contentStatus` — расширяемый enum: `broken` | `needs_work` | `ready`. В моках новые правила по умолчанию `needs_work`. Метка и рамка показываются на каталоге и карточке. `contentNote` — комментарий разработки, не игровой текст.

`contentStatus` не влияет на runtime, validation, публикацию или доступность и не означает «движок умеет исполнить». Runtime-support моделируется отдельно; его форма и полный набор значений требуют согласования с фактическим контентом и имеют статус `OPEN`.
