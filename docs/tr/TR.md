...
> **Статус:** Актуализирован 31.07.2026 — SpaceRevision, клиентский draft, URL-контекст просмотра (`/space/:code/:ctx`), редактирование «читать из контекста → писать в draft»
...
Черты и особенности могут иметь уровни, стоимость, быть отрицательными. Разница между ними — в **контексте использования** (на каком этапе создания доступны, какой валютой оплачиваются). Валюта привязана к этапу (зоне), отдельно валюту не указываем:
- Зона `creation` → ОС (Очки Создания) — расы и врождённые черты
- Зона `personality` → ОЛ (Очки Личности) — особенности личности
- Зона `live` → ОР (Очки Развития) — навыки и черты

**Цена живёт в зонах:** каждая зона несёт свою цену в своей валюте. Зоны нет в `zones` — там способность недоступна. `levels`/`hard`/`automatic` отдельными полями не хранятся.

Поля способности в `spec`:

```typescript
type ZoneId = 'creation' | 'personality' | 'live'
...
### Правила
19. ✅ Валюты — версионируемая сущность
20. ✅ Раса — тип-контейнер с `parent_race_id`
...
30.18. ✅ Способность: цена живёт в зонах (`zones: Partial<Record<ZoneId, AbilityCost>>`), `AbilityCost` = array/progression/automatic; `levels`/`hard`/`automatic` как отдельные поля убраны
