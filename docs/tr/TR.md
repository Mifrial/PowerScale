...
1. **Добить текущий шаг проверки** — попадание ББ 1v1: реакция у защитника, размерная эффективность — ✅; увечье (оферта с карточки, типы урона с хуками) — ✅ 2026-08-23; SSE — дальше.
2. **Актуализировать доки** — ТР и `docs/specs/` под код после полировки.
3. **Коммит и пуш** доработок проверки/карточки — ✅ 2026-08-23.
4. **Ревью + рефактор Character** — порядок кода, мёртвые пути, согласованность листа/оверлея/обзора.
5. **Ревью + рефактор Game** — чат, бой, оферты, карточка.
6. **Ревью + правки Rule** — каталог, спеки, валидация.
7. **Глобальное ревью** — критерии отдельно (имена, границы модулей, дубли, тесты).
8. **Импорт/экспорт ревизии** — файл ревизии → в пространство или новое пространство.
9. **Модуль уведомлений** — сейчас полузаглушка: отправка с кнопками, реакции, подвеска по модулям.
10. **Бэк** — когда начинать и как: после стабилизации фронтовых границ (после ревью модулей или параллельно с уведомлениями). Ожидать переделку API/моков.
11. **Приведение правил к реальному виду + полировка** — несколько этапов: заклинания; большая пачка Видов/Рас; навыки из заглушек (кривой импорт описания) в рабочее состояние.
12. **Плейтест полумакета** — дать игрокам пощупать сборку (вероятно через GitHub) и собрать предложения/замечания.
13. **UX инициативы** — при старте шкалы автоматически все активные персонажи игры; лишних можно убрать, чтобы не искать каждого.
14. **Автомодерация без изменений** — переход версии персонажа (миграция / pending), если diff пустой — принимать без ручной модерации.
15. **Клик-атака** — ✅ 2026-08-23 клик по боевому профилю открывает окно атаки (handshake попадания): выбор действия-атаки (автоматические простые ББ/ДБ, ОД из `action_components`); реакции защиты 0/1/2 ОД; оплата X РУ; урон `(оружие − сопротивление) × РУ`; защита в сопротивление, если нет `defense_ignored`; чат из 4 сообщений; списание ОД и истощения на оверлей.
16. **Модерация только своей ревизии** — ✅ нельзя одобрить персонажа, чей лист не на ревизии игры; такие подачи автоотклонять.
17. **Переподача после отклонения** — ✅ rejected может править лист / перевести на ревизию игры и подать снова, не выходя из игры.
18. **Полировка чата игры** — следующий заход: UX сообщений, вложения расчёта удара, мета-кнопки, согласованность с клик-атакой.
19. **ОД в сценах** — на старте инициативы у всех максимум ОД; в конце хода ОД восстанавливаются до полных значений.
20. **Естественное оружие** — минимум руки и ноги как врождённые атаки без предмета.
21. **Версионность НПС** — механизм актуализации листов НПС при переходе игры на другую ревизию правил.
