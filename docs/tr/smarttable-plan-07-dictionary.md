...
Кэш. Накат схемы репозитория (не этот модуль). Генерация PHP-файлов. ACL. Админка. Переименование. ALTER TYPE. Составной unique — [`smarttable-plan-16-composite-unique.md`](smarttable-plan-16-composite-unique.md) (класс и словарь). Регистрация всех PHP-классов таблиц в словаре. `open(class)` ищет словарь. LIKE по mfv. Подпапки `Service/`. Молчаливый ingest «физика есть, строки словаря нет». Hydrator JSON-поля в словаре (код в реестре).
