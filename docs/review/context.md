...
- **Публикация подмножества правил галочками** (work_item F16–F18 из ТЗ, в этой таблице — не строки F16/F17/F18, они «РЕШЕНО»): коммит только выбранных, неотмеченные остаются в черновике (ТР §5). Остаётся отложенным.
...
| F35 | ~~Vuetify-2 пропсы `outlined`/`dense` в CharacteristicEditor (Vuetify 3 — `variant`/`density`)~~ **РЕШЕНО 2026-08-02** — заменены в 3 местах (внимание: это **другой** F35, чем ТЗ-F35 «inline-чипы» — см. примечание ниже) | LOW | проблема кода | Rule |
...
> **Примечание (2026-08-05) — коллизия нумерации «F35»:** в этой таблице F35 —
> Vuetify-пропсы (Rule). В ТЗ/ТР и волнах Chat (context-13/14) «F35» — inline-чипы
> в чате (ТР §13 №20). Это разные задачи с совпавшим номером; контекст различается
> по документу. Аналогично: отложенная «публикация подмножества правил» — это
> work_item ТЗ F16–F18, а не строки F16/F17/F18 этой таблицы.
...
### Открытые вопросы после F1 (известное, отслеживается)
...
> Статус этих двух вопросов не менялся с 2026-08-01; они зафиксированы как
> известные ограничения mock-эры, будут пересмотрены при бэкенд-контракте.
