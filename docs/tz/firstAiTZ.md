...
# Проект: PowerScale (рабочее название)

## Этапы работы

### Этап 0 — Фундаментальная архитектура (проработан, решения перенесены в ТЗ)
ORM (Doctrine → SmartTable — единственная точка доступа), миграции (схема → SmartTable, data → SmartTable/DoctrineMigrations), архитектура проекта (движок/модули/фронт Shell+модули), мультиязычность, обработка ошибок, логирование через БД (таблица `log`, SmartTable), деплой/бэкапы.

**Защита от CSRF (принято):** Synchronizer Token Pattern. Бэк выдаёт `csrf-token` cookie (non-httpOnly, SameSite=Strict) при логине. Фронт читает cookie и шлёт его в заголовке `X-CSRF-Token` на каждый не-GET запрос. Бэк сверяет header === cookie. /api/login — исключение (CSRF не требуется, т.к. нет сессии).
...
```
[Черновик] → [Готов] → (принят в игру) → Готов (game_id)
                                            ↓ (переход на новую версию)
                     [Модерация] → если проблемы → [Требует исправления]
                           ↓                           ↓
                    всё ок → [Готов]     исправлен → [Модерация]
```
...
**Решение: Подход 2 — всё в JSON (spec_json):**
```
rule_versions(rule_id, space_id, type, name, description_html, spec_json, active, created_at)
```
Типоспецифичные поля — внутри `spec_json` (JSON). SmartTable-поле `JsonField` с hydrator'ом автоматически преобразует JSON в PHP-объект и обратно. Валидация — в hydrator'е.

В столбцах: поля, необходимые для поиска/фильтрации/индексов (`type`, `active`, `name`, `created_at`). В `spec_json`: вся механика (cost, hard, levels, attacks, формулы, требования и т.д.).

(+) Максимальная гибкость — новый тип = новая JSON-схема, без миграций
(+) Никаких дополнительных таблиц
(+) Индексы по type/active/name/space_id — возможны
(+) JsonField + hydrator — на выходе типизированный объект, не сырой JSON
(+) С MySQL 8+ / MariaDB 10.6+ JSON-функции производительны для объёмов правил (сотни — тысячи)
(−) Фильтрация по полям внутри JSON — через JSON-функции БД (не критично для тысячи записей)
(−) Валидация на уровне приложения (hydrator)
...
### Мульти-профиль (отложено)

Возможность одновременного входа в несколько аккаунтов с переключением между ними через меню в топбаре.
**Решение: отложено.** Реализация в init-релизе нецелесообразна — сложность не оправдана. Для тестирования достаточно инкогнито/второго браузера.
...
- **Шапка:** аватар, имя, раса, игра, статус версии (черновик/готов/на модерации/требует исправления), игрок. Маркер «в игре» — по наличию `game_id`.
...
2. **Модерация персонажа** — UI утверждения/отклонения с полем для комментария ведущего. Комментарий отправляется как сообщение в чат обсуждения персонажа, видимое только модератору и владельцу. В `character_moderation` хранится `(character_version_id, moderator_id, action, chat_message_id, created_at)` — без дублирования текста.
3. **Приглашения**
...
```
(все типоспецифичные поля правил — внутри `rule_versions.spec_json`)
...
- **ORM:** Doctrine ORM (DataMapper). SmartTable — единственная точка доступа к данным. Сущности Doctrine — только внутри SmartTable, наружу — чистая модель. Репозитории используют SmartTable, не Doctrine.
- **SmartTable:** bitrix-подобный API (`ServiceLocator::get('Core.SmartTable.Service.Container')`). Обязан поддерживать: лимит полей, сортировку, группировку, сложную фильтрацию, join'ы (через `MultipleField`/явно), runtime-поля, кэш, индексы (`addIndex`, `addUniqueIndex`, `addFulltextIndex`). Синтаксис: `getList([...])` как основной, `query()` как fluent-надстройка для сложных случаев.
- **Типы полей SmartTable:** `IntegerField`, `StringField`, `TextField`, `MultipleField`, `JsonField`, `DateTimeField` — каждый со встроенным кастом (cast/extract/hydrate). `JsonField` поддерживает внешний hydrator для составных объектов (DimensionalNumber, DamageFormula и т.д.). cast() принимает как объект, так и строку; extract() → значение для БД; hydrate() → объект.
- **Миграции:** Схема — SmartTable API (поля, индексы, связи, переименование). Data-миграции — SmartTable в первую очередь, DoctrineMigrations как fallback. Раннер — DoctrineMigrations. Админ-интерфейс (список, накат, откат).
- **Фронт:** Shell + модули (TypeScript, Vite). ESLint + Prettier.
- **Git:** Git Flow. ТЗ в `develop`, не попадает в `main`.
- **Code-style:** PSR-12 + PHP CS Fixer; TS — строгая типизация. Принципы: SOLID, DRY, KISS. Широкое использование интерфейсов, DTO, enum где оправдано.
- **Архитектура кода:** Controller/Event/... → Service → Repository → SmartTable. Репозитории выполняют конкретные задачи, не кастомизируют входящие данные и не используют другие репозитории.
- **Деплой:** Deployer (SSH, zero-downtime). Бэкапы: cron + скрипт (БД + файлы).
- **Ошибки:** `ErrorHandlerInterface`, по умолчанию в БД. Фронт — `window.onerror` + перехват fetch.
...
**Компонент** — связанная пара PHP-контроллера (или группы) и Vue-компонента, объединённых единой задачей. Связь через типизированный API-контракт.

**Цикл работы компонента:**
1. Shell-роутер определяет модуль и монтирует Vue-компонент
2. Компонент вызывает `engine.runAction('module.controller.action', params)`
3. Диспатчер (`/api/dispatch.php`) проверяет авторизацию, CSRF, логирует в БД (таблица `log`) → роутит на контроллер
4. Контроллер обрабатывает, возвращает `RunActionResult`
5. Компонент рендерит результат

**Формат ответа `runAction`:**

```typescript
interface RunActionResponse<T = any> {
  success: boolean;
  data?: T;
  error?: {
    code: string;          // 'ACCESS_DENIED', 'VALIDATION_ERROR' и т.д.
    message: string;       // человекочитаемое описание
    fields?: Record<string, string>;  // ошибки по полям для форм
  };
}
```

- HTTP-статус всегда 200, ошибка в теле — единая обработка на фронте
- Авторизация, CSRF, логирование (в таблицу `log` через SmartTable) — централизованно в диспатчере
- Контроллеры регистрируются с маппингом `module.controller.action` → `ControllerClass::method`

### Переиспользование компонентов — EntitySelector

Базовый механизм: композабл `useEntitySelector(entity)` (поиск, пагинация, выделение). На нём строится компонент `EntitySelector` со слотами (заголовок, список, чипсы выбранного). Узкоспециализированные селекторы (`RaceTreeSelector`, `GameSelector`, `UserSelector` и т.д.) — через слоты. Если слотов не хватает — `useEntitySelector()` напрямую.
...
**UI-фреймворк:** Vuetify. Типовые компоненты (ExpansionPanel, Select, Dialog, Table, Filter, и т.д.) — нативные Vuetify-компоненты с project-specific обёрткой, не изобретаются с нуля.

**Slider** — универсальный слайдер (панель, выезжающая справа). Используется для просмотра карточки пользователя/игры/правила без ухода со страницы. Контент — произвольный компонент через слот. Закрытие по ✕ или overlay. Структура: заголовок (название + кнопка закрытия) + тело (передаваемый компонент) + опциональный футер.

**ExpansionPanel** — аккордеон с заголовком (имя + ✏️ + стоимость) и сворачиваемым телом. Редактор персонажа, правила.

**DiffViewer** — подсветка изменений (добавлено/удалено). Публикация правил, версии персонажа.

**RuleEditor** — форма по типу правила (общие поля + спецификация JSON-блоками).

**StatusBadge** — цветной чип статуса (черновик🟡, готов✅, на модерации🔵, требует исправления🟠). Для персонажа в игре — пиктограмма игры + статус версии.

**ConfirmDialog** — модальное подтверждение с опциональным текстом.

**TagChip** — кликабельный чип тега с цветом.

**InlineEdit** — клик по тексту → поле ввода → save/cancel.

**EditorPanel** — горизонтальные табы для редактора персонажа (Раса → Основа → Личность → Развитие → Инвентарь).

**TooltipPopover** — всплывающая подсказка при ховере на ссылку правила/термина.

**Фильтры (Bitrix-style)** — инпут с иконкой лупы, чипсы внутри слева, попап с тумблерами полей. Стандартный паттерн для всех списков.

**Сайдбар** — collapsible (rail mode, expand-on-hover). Блок пользователя сверху, пункты меню (иконка+текст): Персонажи, Игры, Пользователи, Пространства.

**Топбар** — кнопка ≡ (сайдбар) слева, топ-меню по центру, аватар+имя справа.

**Аватар** — у пользователей и персонажей. Сжатие (общий механизм). Мини-версия (кружок) с выбором области кадрирования.
...
### Система бросков (Rule Engine)

Бросок — конвейер шагов-модификаторов. Каждый шаг реализует `DiceModifier`:

```php
interface DiceModifier {
    public function apply(DiceRoll $roll): DiceRoll;
    public function getKey(): string;            // 'rule_six_one'
    public function getVersion(): string;        // 'v2'
    public function getName(): string;           // "Правило 6 и 1 (новая версия)"
...
**Типовые шаги:** преимущества/помехи, правило 6/1, спецправила (увечья, колющее оружие, критические удары).

**Версионирование:** цепочка определяется версией правил пространства. Игра выбирает версию → конвейер собирается из модификаторов, актуальных для этой версии. Старая игра → `rule_six_one(v1)`, новая → `rule_six_one(v2)`.

**UI:** селектор модификатора (ключ + версия + имя + краткое описание). Кнопка «?» → попап с полным описанием и примером.
...
## Приложение C — Организация документации

```
/docs/
  release1/                      # init-релиз
    makets/                      # копия макетов
    origin_rules/                # docx-файлы оригинальных правил
    finalAiTZ.md                 # итоговое ТЗ
    finalAiTR.md                 # итоговое ТР
    finalTZ.pdf                  # ТЗ для человека
    finalTR.pdf                  # ТР для человека
    hotfixes/                    # хотфиксы релиза
      hotfix-001.md
      ...
  actualPR.pdf                   # актуальное проектное решение
  oldPR/                         # старые версии PR
    PR_r1.pdf
    PR_r2.pdf
```

- При каждом новом релизе: `releaseN/` создаётся заново
- `actualPR.pdf` обновляется (старая версия → `oldPR/PR_rN.pdf`)
- Хотфиксы добавляются в `releaseN/hotfixes/`. При накоплении → инкремент версии
